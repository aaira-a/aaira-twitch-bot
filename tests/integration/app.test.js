...
describe('GET /bot/search-song', () => {
  ...
  it('should return search result data from Spotify', () => {
    ...
    const scope = nock("https://api.spotify.com")
        .get('/v1/search?q=my search query 1&type=track&limit=1')
        .reply(200, mockValidSpotifyResponse)

    return request(app)
      .get('/bot/search-song?query=my search query 1')
      .then((response) => {
        expect(response.status).to.eql(200)
        expect(response.headers['content-type']).to.include('application/json');
        expect(response.body).to.eql(expectedResponse);
...
describe('GET /bot/skip', () => {
  ...
  it('should skip track in Spotify', () => {
    ...
    let expectedResponse = "Skipped song";
